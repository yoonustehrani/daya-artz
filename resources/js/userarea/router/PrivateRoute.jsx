    ...
    render() {
        ...
        return (
            <Route path={path} exact={exact} render={({history, location, match}) => (user
                ? location.pathname === "/" ? <Redirect to={{ pathname: "/dashboard", state: {from: location} }} /> : <>{CallableComponent && <CallableComponent history={history} location={location} {...match} {...this.props} />}{children}</>
